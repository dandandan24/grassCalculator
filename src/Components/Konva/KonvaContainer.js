...
const KonvaContainer = (props) => {
        ...
        if(props.mode === 'Polygon'){
            ...
            else
            {
                ...
                if (isMouseOverStartPoint && props.points.length >= 3) {
                    ...
                    props.actionTrigger()
                    console.log(props.stateStack , 'stateStack')
                }
        }
        }
        if(props.mode === 'Rect'){
    ...
    const onMouseUp = () => {
        ...
        if(props.mode === 'Circle'){
            ...
            props.UpdateCircles([
                ...props.Circles,
                {...currentCircle ,radius :PitagorasSentence(x,y,x0,y0)*2,width : PitagorasSentence(x,y,x0,y0)*2,height : PitagorasSentence(x,y,x0,y0)*2, id : 'Circle'+props.Circles.length.toString() , stroke  : 'black',strokeWidth : 2}                    
            ])
            props.actionTrigger()
            }
    ...
    const handleDragMovePoint = event => {
        if(DraggedPoint !== null) {
            ...
            let anchorDots = [];
            console.log(props.Polygons[DraggedPolygon]['anchorPoints'] , 'at start')
            flattenddots.map((point,index) => {
                if(index % 2 === 0 && index < flattenddots.length - 3 ){
                    console.log(index)
                    console.log(props.Polygons[DraggedPolygon]['anchorPoints'])
